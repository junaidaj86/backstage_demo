...
export const ExampleFetchComponent = () => {
  ...
  const { value, loading, error } = useAsync(async (): Promise<User[]> => {
    const consumerGroupOffsets: ConsumerGroupOffsetsResponse = await kafkaClient.getConsumerGroupOffsets();
    
    // Transform the offsets into user data for the table
    return consumerGroupOffsets.offsets.map(offset => ({
      gender: 'unknown', // Placeholder, as gender is not in the offsets
      name: {
        title: 'Mr/Ms', // Placeholder
        first: 'FirstName', // Placeholder
        last: 'LastName', // Placeholder
      },
      email: `@example.com`, // Placeholder email
      picture: 'https://via.placeholder.com/32', // Placeholder picture
      nat: 'US', // Placeholder nationality
    }));
  }, []);
